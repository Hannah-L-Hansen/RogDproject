...
#boxplot 
box_0.5p_1p <- ggplot(data = bmpRate.met0.5p1p, aes(substrate, time.d, fill = calcMethod)) +
  geom_boxplot()+
  facet_grid(test ~ .)  +
...
#bar chart   
bar_0.5p_1p <- ggplot(data = bmpRate.met0.5p1p, aes(lab, time.d, fill = calcMethod)) +
  geom_col(position = 'dodge')+
  facet_grid(test ~ substrate) +
